...
namespace Caltec.StudentInfoProject.Business
{
    public class SchoolFeesService : BaseService
    {
        ...
        {
            var schoolFee = await StudentInfoDbContext.SchoolFees
                .Include(x => x.Student)
                .Include(x => x.Class)
                .FirstOrDefaultAsync(x => x.Id == schoolFeeId, cancellationToken);
            if (schoolFee == null)
            {
        ...
        public async Task UpdateAsync(SchoolFeesDto schoolFeesDto, CancellationToken cancellationToken)
        {
            var schoolFee = await StudentInfoDbContext.SchoolFees
                .Include(x => x.Student)
                .Include(x => x.Class)
                .FirstOrDefaultAsync(x => x.Id == schoolFeesDto.Id, cancellationToken);
            if (schoolFee == null)
            {
                throw new NotFoundException("School Fee not found");
            }
        ...
        {
            var schoolFee = await StudentInfoDbContext.SchoolFees.FirstOrDefaultAsync(x => x.Id == schoolFeeId, cancellationToken);
            if (schoolFee == null)
            {
                throw new NotFoundException("School Fee not found");
            }
